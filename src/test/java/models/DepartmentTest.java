...
public class DepartmentTest {
    ...
    public void getDepartmentNameReturnsNameCorrectly_String() {
        Department newDepartment = setupNewDepartment();
        assertEquals("accounting", newDepartment.getName());
    }

    @Test
    public void setDepartmentNameSetsCorrectDepartmentName() {
        Department newDepartment = setupNewDepartment();
        newDepartment.setName("statistics");
        assertEquals("statistics", newDepartment.getName());
    }

    @Test
    public void getDescriptionReturnsDescriptionCorrectly_String() {
        Department newDepartment = setupNewDepartment();
        assertEquals("handles company's finance", newDepartment.getStory());
    }

    @Test
    public void setDescriptionSetsCorrectDescription_String() {
        Department newDepartment = setupNewDepartment();
        newDepartment.setStory("handles company statistic and research");
        assertEquals("handles company statistic and research", newDepartment.getStory());
    }

    @Test
    public void getNumbersOfEmployeesReturnsNumberOfEmployeesCorrectly_int() {
        Department newDepartment = setupNewDepartment();
        assertEquals(5, newDepartment.getNoOfEmployees());
    }

    @Test
    public void setNumbersOfEmployeesSetsCorrectNumberOfEmployees_int() {
        Department newDepartment = setupNewDepartment();
        newDepartment.setNoOfEmployees(3);
        assertEquals(3, newDepartment.getNoOfEmployees());
    }
